...
const BLINK_VALUE = 0x16 /* 2 byte blink strength */
    ...
    while (pos < payload.length - 1) {
        ...
        switch(cursor) {
            ...
            case(RAW_16_BIT_VALUE):
                ...
                data.raw16 = this.parse2byteUnsignInt(raw16);
                pos+=3;
                break;
            case(RAW_16_BIT_VALUE):
                const blinkValue = payload.slice(pos + 1, pos + 3);
                data.blinkValue = this.parse2byteUnsignInt(blinkValue);
                pos+=3;
                break;
            case(ASIC_EEG_POWER):
                const asicEEG = payload.slice(pos + 1, pos + 25);
                data.eegValues = {
                    delta: this.parse3byteUnsignInt(asicEEG.slice(0, 3)),
                    theta: this.parse3byteUnsignInt(asicEEG.slice(3, 6)),
                    lowAlpha: this.parse3byteUnsignInt(asicEEG.slice(6, 9)),
                    highAlpha: this.parse3byteUnsignInt(asicEEG.slice(9, 12)),
                    lowBeta: this.parse3byteUnsignInt(asicEEG.slice(12, 15)),
                    highBeta: this.parse3byteUnsignInt(asicEEG.slice(15, 18)),
                    lowGamma: this.parse3byteUnsignInt(asicEEG.slice(18, 21)),
                    midGamma: this.parse3byteUnsignInt(asicEEG.slice(21, 24))
                };
                pos+=25;
                break;
            case(PRINTERVAL):
                const prInterval = payload.slice(pos + 1, pos + 3);
                data.rinterval = this.parse2byteUnsignInt(prInterval);
                pos+=3;
                break;
            default:
                pos+=1;
...
MindwaveMobile.prototype.parse2byteUnsignInt = function(buffer){
    return buffer[0] * (8) + buffer[1];
};

MindwaveMobile.prototype.parse3byteUnsignInt = function(buffer){
    return buffer[0] * (256 * 256) + buffer[1] * (256) + buffer[2];
};
